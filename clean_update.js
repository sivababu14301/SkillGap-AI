...
const rolesData = {
    ...
    "MANAGEMENT": [
        {name: "HR Executive (Non-IT)", skills: ["Recruitment", "Employee Relations", "Onboarding", "HR Policies", "Payroll"]},
        ...
        {name: "Project Manager (Non-IT)", skills: ["Project Planning", "Agile/Scrum", "Risk Management", "Budgeting", "Stakeholder Management"]},
        {name: "Product Manager (Non-IT)", skills: ["Product Strategy", "Market Research", "Roadmapping", "Agile", "User Experience"]},
...
nonItRolesNew.push(...existingRoles);
...
// Strip existing nonItRoles completely
let idx = jsContent.indexOf("const nonItRoles = [");
if (idx !== -1) {
    jsContent = jsContent.substring(0, idx);
}

jsContent += "const nonItRoles = " + JSON.stringify(nonItRolesNew, null, 4) + ";\n";
jsContent += "jobRoles.push(...nonItRoles);\n";
fs.writeFileSync("job-roles-data.js", jsContent);

// Dump JSON for seed.py
const { execSync } = require('child_process');
fs.writeFileSync("temp_eval.js", "let window = {};" + jsContent + "\nconsole.log(JSON.stringify(jobRoles));");
