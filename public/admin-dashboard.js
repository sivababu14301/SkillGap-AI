...
function exportData() { exportUsersCSV(); }

function exportUsersCSV() {
    if(!adminState.users || adminState.users.length === 0) {
        showToast("No users to export.", "error"); return;
    }
    const headers = "ID,Name,Email,Role,Created At\n";
    const rows = adminState.users.map(u => `"${u.id || u._id || ''}","${u.name || u.full_name || ''}","${u.email || ''}","${u.role || 'user'}","${u.created_at || ''}"`).join("\n");
    downloadCSV(headers + rows, "SkillGap_Users.csv");
}

function exportResumesCSV() {
    if(!adminState.resumes || adminState.resumes.length === 0) {
        showToast("No resumes to export.", "error"); return;
    }
    const headers = "ID,User ID,Skills Count,Created At\n";
    const rows = adminState.resumes.map(r => {
        const skillCount = Array.isArray(r.extracted_skills) ? r.extracted_skills.length : 0;
        return `"${r.id || r._id || ''}","${r.user_id || ''}","${skillCount}","${r.created_at || ''}"`;
    }).join("\n");
    downloadCSV(headers + rows, "SkillGap_Resumes.csv");
}

function exportAnalysesCSV() {
    if(!adminState.analyses || adminState.analyses.length === 0) {
        showToast("No analysis data to export.", "error"); return;
    }
    const headers = "ID,User ID,Selected Role,Match Percentage,Created At\n";
    const rows = adminState.analyses.map(a => `"${a.id || a._id || ''}","${a.user_id || ''}","${a.selected_job_role || ''}","${a.match_percentage || ''}","${a.created_at || ''}"`).join("\n");
    downloadCSV(headers + rows, "SkillGap_Analyses.csv");
}

function exportAllReports() {
    if(!adminState.reports || adminState.reports.length === 0) {
        showToast("No reports to export.", "error"); return;
    }
    const headers = "ID,User ID,Type,Created At\n";
    const rows = adminState.reports.map(r => `"${r.id || r._id || ''}","${r.user_id || ''}","${r.type || 'roadmap'}","${r.created_at || ''}"`).join("\n");
    downloadCSV(headers + rows, "SkillGap_Reports.csv");
}

function exportAnalyticsReport() {
    const blob = new Blob([JSON.stringify(adminState.stats || {}, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.setAttribute('download', "SkillGap_Analytics.json");
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
