...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Dynamic Active Sidebar and Breadcrumb Logic
    if (sidebar) {
        let currentPath = window.location.pathname.split('/').pop();
        if (!currentPath || currentPath === "" || currentPath === "/") currentPath = "dashboard.html";
        ...
        const activeLinks = sidebar.querySelectorAll(`.nav-link[href^="${currentPath}"]`);
        ...
        if (breadcrumb) {
            const pageInfo = SEARCH_PAGES.find(p => currentPath.includes(p.url));
            if (pageInfo) {
                const span = breadcrumb.querySelector("span");
                if (span) {
                    let title = pageInfo.title;
                    if (pageInfo.url === 'job-role.html') title = "Job Role Selection";
